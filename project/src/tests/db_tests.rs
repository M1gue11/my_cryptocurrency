...
mod tests {
    ...
    fn test_mempool_operations() {
        ...
        // Insert into mempool
        ledger.insert_mempool_tx(&tx).unwrap();

        // Retrieve it
        let txid = tx.id();
        let retrieved = ledger.get_transaction(&txid).unwrap();
        assert!(retrieved.is_some());

        // Remove from mempool
        ledger.remove_mempool_tx(&txid).unwrap();
        let retrieved = ledger.get_transaction(&txid).unwrap();
    ...
    fn test_get_utxos_for_addresses_empty_list() {
        ...
        let utxos = ledger.get_utxos_for_addresses(&Vec::new()).unwrap();
    ...
    fn test_apply_block_genesis() {
        let mut db = create_test_db(Some(":memory:")).unwrap();
        db.init_schema().unwrap();
        let ledger = db.ledger();
        ...
        ledger.apply_block(block, &[tx]).unwrap();
        ...
        let utxos = ledger.get_utxos_for_address("miner_address").unwrap();
        assert_eq!(utxos.len(), 1);
        assert_eq!(utxos[0].output.value, 50.0);
