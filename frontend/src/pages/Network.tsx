...
export function Network() {
  ...
  const [disconnectingPeer, setDisconnectingPeer] = useState<string | null>(
    null,
  );
  ...
  const handleConnectPeer = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();

    const address = newPeerAddress.trim();
    if (!address) {
      setActionMessage("Inform a peer address before connecting.");
      return;
    }
    ...
    try {
      ...
      setActionMessage(null);
      const response = await rpcClient.nodeConnect(address);

      if (!response.success) {
        setActionMessage(
          response.fail_message ?? `Failed to connect to ${address}`,
        );
        return;
      }

      setActionMessage(`node_connect -> connection requested for ${address}`);
      setNewPeerAddress("");
      await loadRuntime(true);
    } catch (nextError) {
      setActionMessage(
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 xl:grid-cols-3">
        ...
        <ConsolePanel title="connect peer" subtitle="node_connect" icon="+">
          <form className="space-y-4" onSubmit={handleConnectPeer}>
            <div>
              <div className="crm-field-label">peer address</div>
              <input
                className="crm-input"
                value={newPeerAddress}
                onChange={(event) => setNewPeerAddress(event.target.value)}
                placeholder="18.116.162.147:6000"
              />
              <div className="mt-2 text-xs leading-5 text-[var(--crm-dim)]">
                Use the full P2P socket address, including port.
              </div>
            </div>
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <ConsolePill
                        tone={
                          peer.direction === "outbound" ? "accent" : "neutral"
                        }
                      >
                        {peer.direction}
                      </ConsolePill>
                      <ConsolePill
                        tone={
                          peer.connection_state === "disconnecting" ?
                            "warn"
                          : "good"
                        }
                      >
                      ...
                      <ConsolePill
                        tone={
                          peer.handshake_state === "handshake_complete" ? "good"
                    ...
                    <div className="grid gap-3 text-sm text-[var(--crm-dim)] md:grid-cols-3">
                      ...
                      <div>
                        last event: {formatTimestamp(peer.last_event_at)}
                      </div>
      ...
      {confirmAction ?
        ...
        >
          ...
          >
            <div className="crm-eyebrow text-[var(--crm-warn)]">
              confirm action
            </div>
  ...
  return (
    <div className="mb-3 rounded-sm border border-[var(--crm-border)] bg-[var(--crm-panel-2)] p-4 last:mb-0">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="text-sm font-medium text-[var(--crm-fg)]">
            {title}
          </div>
